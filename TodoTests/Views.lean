...
/-- `Todo.Views`' functions are generic over `{Urls} [Todo.HasUrls Urls]` (`Todo/Urls.lean`) --
`Main.lean`'s macro-generated `AppUrls` supplies the real instance, but `Main` is a `lean_exe`
target, not a `lean_lib`, so it isn't importable from this test library (`lakefile.toml`). This
sample stands in, matching `Main.lean`'s actual route patterns 1:1 so the expected strings below
stay correct. -/
private structure SampleUrls where

instance : HasUrls SampleUrls where
  index _ := "/"
  active _ := "/active"
  completed _ := "/completed"
  todos _ := "/todos"
  todo _ id := s!"/todos/{id}"
  todoEdit _ id := s!"/todos/{id}/edit"
  todoToggle _ id := s!"/todos/{id}/toggle"
  toggleAll _ := "/todos/toggle-all"
  clearCompleted _ := "/todos/completed"

private def sampleUrls : SampleUrls := {}
...
#guard Node.render (itemView sampleUrls sampleItem) =
...
#guard Node.render (itemView sampleUrls sampleItemDone) =
...
#guard Node.render (itemView sampleUrls sampleItemUnsafe) =
...
#guard Node.render (itemEditView sampleUrls sampleItem) =
...
#guard Node.render (itemEditView sampleUrls sampleItemUnsafe) =
...
#guard Node.render (listSection sampleUrls #[]) =
...
#guard Node.render (listSection sampleUrls #[sampleItem]) =
...
#guard Node.render (listSection sampleUrls #[sampleItemDone]) =
...
#guard Node.render (filterLink sampleUrls .all .all "All") = "<li><a href=\"/\" class=\"selected\">All</a></li>"
#guard Node.render (filterLink sampleUrls .all .active "Active") = "<li><a href=\"/active\">Active</a></li>"
...
#guard Node.render (footerFragment sampleUrls #[] .all) =
...
#guard Node.render (footerFragment sampleUrls #[sampleItem] .active) =
...
#guard Node.render (footerFragment sampleUrls #[sampleItem, sampleItemDone] .all) =
...
#guard filterFromPath (Filter.path sampleUrls .all) == .all
#guard filterFromPath (Filter.path sampleUrls .active) == .active
#guard filterFromPath (Filter.path sampleUrls .completed) == .completed
